...
<body>
	...
	<div id="page-wrapper">
					...
					<div>
						...
						<ul class="metisMenu nav" id="side-menu">
							...
							<li><a href="Solicitudes.php"><span class="label label-custom pull-right"></span> <i class="mdi mdi-file-outline"></i> Solicitudes</a></li>
							 <li><a href="Usuarios.php"><span class="label label-custom pull-right"></span> <i class="mdi mdi-account-plus"></i>Registrar Usuarios</a></li>
        ...
        <!-- START PAGE CONTENT -->
			...
			<h4 class="header-title m-t-0 ">Registro de Horarios</h4>
                                    <p class="text-muted font-13 m-b-10">
                                        Favor de llenar todos los campos obligatorios
                                    </p>

                                    <div class="p-20 m-b-20">
                                    <form class="form-horizontal" action=""  method="post" name="registroProfesor">
                                           
                                            <div class="form-group">
                                                <label for="id_falla">Persona <span class="text-danger">*</span></label>
                                                <select id="id_persona" name="id_persona" class="form-control select2 select2-hidden-accessible" tabindex="-1" aria-hidden="true">
																								<?php
																									
																									require("../Config.php");
																									$mysqli = new mysqli(DB_SERVER, DB_USER, DB_PASSWORD, DB_DATABASE);
							
																								//Ejecuto la consulta y si entra al if quiere decir que no se pudo realizar y finalizamos el programa
																								if ($mysqli->connect_error) { 
																									printf("Conexión fallida: %s\n", $mysqli->connect_error);
																									exit();
																								}
																								...
																								{
																									$consulta = "SELECT id_persona, nombre FROM PERSONAS";
                                                      ...
                                                      while ($p = $result -> fetch_assoc()) { //El result me arroja los datos de una fila y los coloco en $p ya con eso acceso a los valores de un restaurante a la vez
                                                        ...
                                                        $mysqli->close(); //Cerramos la conexion
																			           ...
																			           <div class="form-group">
                                                <label for="id_persona">Id Grupo <span class="text-danger">*</span></label>
                                                <select id="id_persona" name="id_persona" class="form-control select2 select2-hidden-accessible" tabindex="-1" aria-hidden="true">
																								<?php
																								...
																								{
																									$consulta = "SELECT id_subdireccion from SUBDIRECCIONES";
               																		
																									if($result = $mysqli->query($consulta)){ 
                                                      while ($p = $result -> fetch_assoc()) { 
	                                              
                                                			echo"<option value=".$p['id_subdireccion'] ."</option>";
                                                        ...
                                                        $mysqli->close(); //Cerramos la conexion
																									...
																									?>
																									 ...
																									 <div class="form-group">
                                                <label for="sexo">Día <span class="text-danger">*</span></label>
                                                <select id="sexo" name="sexo" class="form-control select2 select2-hidden-accessible" tabindex="-1" aria-hidden="true">
                                            ...
                                            <option value="Sabado">Sabado</option>
																			 ...
																			 <div class="form-group">
                                                <label for="sexo">Activo <span class="text-danger">*</span></label>
                                                <select id="sexo" name="sexo" class="form-control select2 select2-hidden-accessible" tabindex="-1" aria-hidden="true">
                                            ...
                                            <div class="form-group text-right m-b-0">
                                                <button class="btn btn-success"id="registroPersona"  name="registroProfesor" type="submit">
                                                    Registrar
                                                </button>
                                            </div>
              ...
              <?php 
			if(isset($_POST['registroPersona']))//Vallidamos que el formulario fue enviado
      {
				if(($_POST['nombre'] != '') && ($_POST['sexo'] != ''))
        {
					require("../Config.php");
					...
					$nombre = $_POST['nombre'];
					$sexo = $_POST['sexo'];
					$consulta = "INSERT INTO Personas (Nombre, Sexo) VALUES ('$nombre', '$sexo')";
					if($mysqli->query($consulta)=== TRUE){ //En caso de que la consulta sea correcta inserta los datos y se regresa a AltaClientes.php
    				//sleep(20);
